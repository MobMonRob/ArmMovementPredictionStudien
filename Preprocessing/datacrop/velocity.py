...
def calculate_velocity_vector_for_dataset(filename, directory="./data/"):
    dataset = open_dataset_pandas(filename, directory)
    return gradient(dataset.to_numpy(), axis=0)


def generate_velocity_dataframe(filename):
...
def write_velocity_dataset_to_csv(filename):
    dataframe_velocity = generate_velocity_dataframe(filename)
